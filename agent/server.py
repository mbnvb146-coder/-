...
async def process(job_id: str, whisper: str = "tiny"):
    """SSE endpoint — streams 4-step pipeline."""
    matches = list(UPLOAD_DIR.glob(f"{job_id}.*"))
    if not matches:
        return JSONResponse({"error": "job not found"}, status_code=404)

    return StreamingResponse(
        _pipeline_sse(job_id, str(matches[0]), whisper),
...
async def download(job_id: str):
    """Return CapCut draft folder as a zip archive."""
...
async def _pipeline_sse(
    ...
    try:
        ...
        yield await _send("step", {
            ...
            "total": round(total, 2), "silences": len(silences), "cut_secs": round(cut_secs, 2),
        })
        ...
        if asr_ok:
            yield await _send("step", {
                ...
                "detail": f"단어 {sum(len(s['text'].split()) for s in segments)}개",
        ...
        yield await _send("step", {
            ...
            "fillers": len(fillers), "keep_count": len(keep_intervals), "edited_dur": round(edited_dur, 2),
        ...
        ratio = round((total - edited_dur) / total * 100, 1) if total > 0 else 0
